-- Pangisaug schema — reconstructed from src/lib/db.ts INSERT/UPDATE column lists (the app's own authority)
-- Idempotent (CREATE TABLE IF NOT EXISTS) so db:seed/bootstrap can run safely.
...
CREATE INDEX IF NOT EXISTS "Payment_resource_idx" ON "Payment"(reference);
